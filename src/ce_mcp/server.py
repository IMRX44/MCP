...
async def _call(route: str, **params: Any) -> Any:
    try:
        return await client().call(route, **params)
    except BridgeError as exc:
        # Surface a clean, actionable message to the model.
...
async def process_list(filter: str | None = None) -> Any:
    ...
    return await _call("/process/list", filter=filter)
...
async def memory_read(
    ...
    """
    return await _call(
        "/memory/read", address=address, type=type, size=size,
...
async def memory_read_batch(reads: list[dict]) -> Any:
    ...
    """
    return await _call("/memory/read_batch", reads=reads)
...
async def memory_write(address: str, value: Any, type: str = "4byte", wide: bool = False) -> Any:
    ...
    """
    return await _call("/memory/write", address=address, value=value, type=type, wide=wide)


@mcp.tool()
async def memory_dump(address: str, size: int = 256) -> Any:
    """Hex-dump a region of memory (16 bytes/row, hex + ASCII). Max 64 KiB."""
    return await _call("/memory/dump", address=address, size=size)
...
async def memory_alloc(size: int = 4096, near: str | None = None) -> Any:
    ...
    return await _call("/memory/alloc", size=size, near=near)


@mcp.tool()
async def memory_free(address: str) -> Any:
    """Free memory previously allocated with ``memory_alloc``."""
    return await _call("/memory/free", address=address)
...
async def scan_aob(
    ...
    return await _call("/scan/aob", pattern=pattern, start=start, stop=stop, protection=protection)
...
async def pointer_resolve(base: str, offsets: list[int] | None = None) -> Any:
    ...
    """
    return await _call("/pointer/resolve", base=base, offsets=offsets or [])
...
async def pointer_scan(address: str, max_level: int = 4, max_offset: int = 2048) -> Any:
    ...
    """
    return await _call("/pointer/scan", address=address, max_level=max_level, max_offset=max_offset)
...
async def auto_assemble(script: str, enable: bool = True) -> Any:
    ...
    Example godmode skeleton::
        ...
        dealloc(newmem)
    """
    return await _call("/autoassemble", script=script, enable=enable)
...
async def find_what_writes(address: str, duration: int = 3) -> Any:
    ...
    return await _call("/debugger/find_what_writes", address=address, duration=duration)
...
async def find_what_accesses(address: str, duration: int = 3) -> Any:
    ...
    return await _call("/debugger/find_what_accesses", address=address, duration=duration)


@mcp.tool()
async def breakpoint(address: str, remove: bool = False, size: int = 1) -> Any:
    """Set (or, with ``remove=True``, clear) a breakpoint at ``address``."""
    return await _call("/debugger/breakpoint", address=address, remove=remove, size=size)
...
async def mono_classes(filter: str | None = None) -> Any:
    ...
    return await _call("/mono/classes", filter=filter)
...
async def speedhack(speed: float) -> Any:
    ...
    return await _call("/speedhack/set", speed=speed)
...
async def lua_execute(code: str) -> Any:
    ...
    return await _call("/lua/execute", code=code)
